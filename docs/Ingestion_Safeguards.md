...
**Relation canonicalization (the reliable path for local models).** Local models over Ollama can't self-constrain their relation vocabulary — `SchemaLLMPathExtractor` strict mode returns *empty* (constrained decoding degenerates), and non-strict is *ignored* (free-form run-on labels). So the working approach is **extract free-form (`EXTRACTOR=simple`) + canonicalize**: `CANONICALIZE=true` (default) runs `RelationCanonicalizer`, which embeds each extracted relation label (reusing the `all-MiniLM` embedder — no extra LLM cost) and snaps it to the nearest schema relation type by cosine similarity, falling back to `RELATED_TO` below `canon_threshold` (config, default 0.45). E.g. `ESTABLISHES_VIA_LOGIC_STEPS → PROVES`, `AUTHOREDTEXTBOOK → AUTHORED_BY`. `CANONICALIZE=false` reverts to backtick-only sanitization (raw labels, for comparison). This decouples extraction (which the models do well) from vocabulary enforcement (which they can't).
