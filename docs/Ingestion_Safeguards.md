...
**Thinking mode.** `EXTRACT_THINKING` = `auto` (default) | `on` | `off` controls hybrid-reasoning ("thinking") models. For high-volume structured extraction you want thinking **off** — it otherwise emits reasoning tokens per chunk, which is slow and can muddy output. `auto` disables thinking for thinking-capable models (qwen3, deepseek-r1, gpt-oss, …) and **omits the flag for non-thinking models** like qwen2.5 (sending `think:false` to those can error). Resolved by `_resolve_thinking()` in [`ingest.py`](../ingest.py).
